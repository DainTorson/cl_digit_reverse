...
int * loadData(char * filename, int &inputLength)
{
	ifstream file(filename);
	int * inputData;
	...
	{
		file >> inputLength;
		inputData = (int * )malloc(inputLength*sizeof(int));
...
void reverse(int * hostInput, int * hostOutput, int inputLength)
{
	for(int index = 0; index < inputLength / 2; ++index)
	{
		hostOutput[index] = hostInput[inputLength - 1 - index];
		hostOutput[inputLength - 1 - index] = hostInput[index];
	}

	if(inputLength % 2 != 0)
	{
		hostOutput[inputLength / 2] = hostInput[inputLength / 2];
	}
}

int parallelReverse(int * hostInput, int * hostOutput, int inputLength)
{
	cl_mem deviceInput;
	cl_mem deviceOutput;
	...
	CHECK(cl_error);
	const char * reversesrc = loadKernel("kernel.c");

	cl_program cl_prgm;
	cl_prgm = clCreateProgramWithSource(context, 1, &reversesrc, NULL, &cl_error);
	CHECK(cl_error);
	...
	cl_kernel kernel = clCreateKernel(cl_prgm, "reverse", &cl_error);
	CHECK(cl_error);

	deviceInput = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, inputLength * sizeof(int),
								 hostInput, &cl_error);
	CHECK(cl_error);
	deviceOutput = clCreateBuffer(context, CL_MEM_WRITE_ONLY, inputLength * sizeof(int), NULL, &cl_error);
	CHECK(cl_error);
	...
	clEnqueueReadBuffer(cl_cmd_queue, deviceOutput, CL_FALSE, 0, inputLength * sizeof(int), hostOutput, 0, NULL, NULL);
...
int main(void)
{
	...
	{
		int check = parallelReverse(hostInput, hostOutput, inputLength);
		if(check == -1)
		{
			system("PAUSE");
			return -1;
	...
	{
		reverse(hostInput, hostOutput, inputLength);
	}
